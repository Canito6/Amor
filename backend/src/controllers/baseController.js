 ...
 * for mongoose models scoped to the current couple (req.coupleId).
 */
class BaseController {
  ...
  async getAllItems(req, res, next, query = {}, sort = { createdAt: -1 }) {
    try {
      ...
      if (page && limit) {
        ...
        const items = await this.model.find(filter).sort(sort).skip(skip).limit(limit);
        ...
        const items = await this.model.find(filter).sort(sort);
        res.json(items);
      }
    } catch (error) {
  ...
  async createItem(req, res, next, bodyData) {
    try {
      ...
      const item = new this.model(cleanData);
      await item.save();
