...
export default function useQuizzes(t, language, meuNome) {
  ...
  const submeterNovoQuiz = async (e) => {
    ...
    for (const [index, q] of questions.entries()) {
      if (!q.questionText.trim()) {
        showToast(t.quizzes_alert_empty_question.replace('{num}', index + 1), 'error');
        return;
      }
      if (q.options.some(opt => !opt.trim())) {
        showToast(t.quizzes_alert_empty_option.replace('{num}', index + 1), 'error');
        return;
      }
      if (!q.creatorAnswer) {
        showToast(t.quizzes_alert_no_correct.replace('{num}', index + 1), 'error');
        return;
      }
    }
    ...
    try {
      ...
      showToast(t.quizzes_alert_created_success, 'success');
    } catch (err) {
  ...
  const submeterRespostas = async (e) => {
    ...
    if (currentGuesses.some(g => g === '')) {
      showToast(t.quizzes_alert_unanswered, 'error');
      return;
    }
  ...
  const apagarQuiz = async (id) => {
    const ok = await confirm({
      title: t.quizzes_confirm_delete || 'Apagar quiz?',
      message: t.quizzes_confirm_delete || 'Tens a certeza que queres apagar este quiz?',
